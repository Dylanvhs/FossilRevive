...
@OnlyIn(Dist.CLIENT)
public class MicroraptorOnShoulderLayer<T extends Player> extends RenderLayer<T, PlayerModel<T>> {
    private final Microraptor model;

    public MicroraptorOnShoulderLayer(RenderLayerParent<T, PlayerModel<T>> pRenderer, EntityModelSet pModelSet) {
        super(pRenderer);
        this.model = new Microraptor(pModelSet.bakeLayer(ModModelLayers.MICRORAPTOR_LAYER));
    }
    ...
    private void render(PoseStack pMatrixStack, MultiBufferSource pBuffer, int pPackedLight, T pLivingEntity, float pLimbSwing, float pLimbSwingAmount, float pNetHeadYaw, float pHeadPitch, boolean pLeftShoulder) {
        ...
        EntityType.byString(compoundtag.getString("id")).filter((p_117294_) -> {
            ...
            this.model.renderOnShoulder(pMatrixStack, pPackedLight, OverlayTexture.NO_OVERLAY, pLimbSwing, pLimbSwingAmount, pNetHeadYaw, pHeadPitch, pLivingEntity.tickCount);
